...
public:
    ...
    // Recursion
    int solveR(vector<vector<char>>& mat, int i, int j, int &maxi) {
    ...
    int solveMemo(vector<vector<char>>& mat,
                  int i,
                  int j,
                  int &maxi,
        ...
        for(int i = row - 1; i >= 0; i--) {
            ...
            for(int j = col - 1; j >= 0; j--) {
                ...
                if(mat[i][j] == '1') {
                    ...
                    dp[i][j] =
                        1 + min(right, min(diagonal, bottom));
